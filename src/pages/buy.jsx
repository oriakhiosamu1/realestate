...
const Buy = () => {
  ...
  // Determine default page mode
  const isBuyPage = location.pathname === '/buy';

  const [listingType, setListingType] = useState(isBuyPage ? 'buy' : 'rent');
  ...
  // Fetch from backend (only property_type + pagination)
  useEffect(() => {
    const fetchHouses = async () => {
      ...
      // Only property_type is backend-filtered
      if (listingType === 'buy' || listingType === 'rent') {
        params.property_type = listingType;
      }
  ...
  const filtered = useMemo(() => {
    return houses.filter(p => {
      ...
      // Beds filter
      if (selectedBeds !== 'Any') {
        const min = parseInt(selectedBeds);
        if (Number(p.beds) < min) return false;
      }

      // Search filter
      if (searchQuery) {
        if (!String(p.location || '').toLowerCase().includes(searchQuery.toLowerCase())) return false;
      }
  ...
  // Pagination (frontend from backend dataset)
  const paginated = filtered;


  // Navigation logic (your requirement)
    ...
    <div className="min-h-screen bg-white">
      <div className="max-w-[1400px] mx-auto px-4 md:px-8 pt-6 pb-4">
        ...
        <div className="flex flex-wrap items-center gap-2 mb-4">
          <button 
            onClick={goBuy}
            className={isBuyPage ? 'px-4 py-2 border border-gray-300 rounded text-sm font-medium text-gray-900 bg-gray-200' : FILTER_BUTTON_CLASSES}
          >
            Buy
          </button>
          ...
          <button 
            onClick={setBoth}
            className={listingType === '' ? 'px-4 py-2 border border-gray-300 rounded text-sm font-medium text-gray-900 bg-gray-200' : FILTER_BUTTON_CLASSES}
          >
            Both
          </button>
        ...
        <div className="flex flex-col md:flex-row max-w-[1400px] mx-auto pb-10">
          ...
          <div className="md:w-1/2 lg:w-2/5 pr-0 md:pr-6 space-y-6">
            ...
            {!loading && paginated.length === 0 && (
              <p className="text-gray-500 py-12 text-center">No properties match your filters.</p>
            )}
