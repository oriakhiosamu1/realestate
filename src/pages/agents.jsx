...
import { Mail, Phone, Search, MapPin, ChevronLeft, ChevronRight } from "lucide-react";
import { motion } from "framer-motion";
import axiosClient from "../axiosClient/axiosClient";

// Shared class for contact buttons
const GOLD_BUTTON_CLASSES =
  "flex-1 inline-flex items-center justify-center gap-1.5 px-3 py-2 text-sm border border-yellow-300 rounded-md text-gray-700 hover:bg-yellow-50 hover:border-yellow-600 transition duration-200";

// Agent Card
const AgentCard = ({ agent, index }) => {
  const imgSrc =
    agent.image ||
    `https://placehold.co/400x500/F9FAFB/333333?text=${agent.name
      .split(" ")
      .map((n) => n[0])
      .join("")}`;
    ...
    >
      <div className="w-1/3 min-w-[120px] max-w-[200px] flex-shrink-0">
        <img
          ...
          onError={(e) => {
            e.target.onerror = null;
            e.target.src = `https://placehold.co/400x500/F9FAFB/333333?text=${agent.name
              .split(" ")
              .map((n) => n[0])
              .join("")}`;
          }}
      ...
      <div className="p-4 sm:p-5 flex flex-col justify-between w-full">
        <div>
          ...
          {agent.languages && agent.languages.length > 0 && (
            <p className="text-xs text-gray-500">
              <span className="font-semibold">Speaks:</span> {agent.languages}
            </p>
          )}
        </div>
...
const Pagination = ({ currentPage, lastPage, onPageChange }) => {
  ...
  let endPage = Math.min(lastPage, startPage + MAX_VISIBLE - 1);
  ...
  for (let i = startPage; i <= endPage; i++) {
    pages.push(
      <button
        key={i}
        onClick={() => onPageChange(i)}
        className={`text-sm px-2 pb-0.5 font-medium transition duration-200 ${
          i === currentPage ? "border-b-2 border-yellow-600" : "text-gray-700 hover:text-yellow-600"
        }`}
      >
        {i}
      </button>
  ...
  if (endPage < lastPage) {
    pages.push(
      <span key="ellipsis" className="text-sm px-2 text-gray-500 pb-0.5">
        ...
      </span>
    );
    pages.push(
  ...
  const ChevronButton = ({ direction, disabled, onClick }) => {
    ...
    return (
      <button
        ...
        disabled={disabled}
        className={`w-10 h-10 flex items-center justify-center rounded-full border-2 transition duration-300 ${
          disabled ? "border-gray-200 text-gray-400" : "border-yellow-300 text-yellow-600 hover:bg-yellow-50"
        }`}
      >
        <Icon size={20} />
      </button>
  ...
  return (
    <div className="flex justify-center items-center space-x-3 mt-10">
      <ChevronButton direction="left" disabled={currentPage === 1} onClick={() => onPageChange(currentPage - 1)} />
        <div className="flex items-end space-x-3 h-10">{pages}</div>
      <ChevronButton direction="right" disabled={currentPage === lastPage} onClick={() => onPageChange(currentPage + 1)} />
    </div>
  );
};
...
const App = () => {
  ...
  const fetchAgents = async (page = 1) => {
    try {
      const params = {
        page,
        search: searchTerm,
        master_circle: isMasterCircle ? 1 : 0,
      };
      const res = await axiosClient.get("/agents", { params });
      const data = res.data;
      setAgents(data.data || []); // Laravel pagination data
  ...
  return (
    <div className="min-h-screen bg-white font-sans">
      <header className="pt-8 pb-4 border-b border-gray-200">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <h1 className="text-3xl font-serif text-gray-900 tracking-tight">Real Estate Agents Worldwide</h1>
        </div>
      </header>

      <main className="max-w-7xl mx-auto py-8 px-4 sm:px-6 lg:px-8">
        {/* Search + Master Circle */}
        <div className="flex flex-col md:flex-row items-center justify-between gap-4 mb-6">
          <div className="flex-grow flex w-full md:w-auto">
            <input
              type="text"
        ...
        {/* Results Count */}
        <div className="mb-6 py-2 border-b border-gray-200">
          <p className="text-base font-semibold text-gray-700">{agents.length} Agents</p>
        </div>

        {/* Agent Grid */}
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          {(agents || []).length > 0 ? (
            (agents || []).map((agent, index) => <AgentCard key={agent.id || agent.license} agent={agent} index={index} />)
          ) : (
            <div className="lg:col-span-2 text-center py-16 bg-gray-50 rounded-lg border border-gray-200 text-gray-500">
              <Search size={32} className="mx-auto mb-4" />
              <p className="text-xl font-medium">No agents match your criteria.</p>
