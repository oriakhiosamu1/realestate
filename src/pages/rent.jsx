...
//           {listing.beds} Beds · {listing.baths} Baths · {listing.sqft} ft²
//         </p>
//         <p className="text-sm text-gray-500 truncate">{listing.address}</p>
//       </div>
//     </div>
  ...
  return (
    ...
    >
      <div className="relative">
        ...
        {property.featured && (
          <div className="absolute bottom-4 left-4 bg-white text-gray-800 text-xs font-semibold px-3 py-1 rounded-full shadow-lg flex items-center">
            <Star size={12} style={{ color: GOLD_COLOR }} className="mr-1" fill={GOLD_COLOR} />
            Featured Listing
          </div>
        )}
      ...
      <div className="p-4 border-t border-gray-100 flex-grow">
        <p className="text-xl font-light text-gray-900 mb-1">
          {property.name}
        </p>
        <p className="text-xl font-light text-gray-900 mb-1">
          {property.price ? `$${Number(String(property.price).replace(/,/g, '')).toLocaleString()}` : 'Price on request'}
        </p>
        <p className="text-sm text-gray-600 mb-2">
...
const Rent = () => {
  ...
  useEffect(() => {
    const fetchHouses = async () => {
      ...
      const params = { page: pageNumber, property_type: listingType };

      try {
        const res = await axiosClient.get('/houses', { params });
    ...
    fetchHouses();
  }, [listingType, pageNumber]);

  // Reset pagination when filters change
  useEffect(() => { setPageNumber(1); }, [listingType]);

  // FRONTEND FILTERING
  const filtered = useMemo(() => {
    return houses.filter(p => {
      if (selectedType !== 'Any' && (p.property_type || '').toLowerCase() !== selectedType.toLowerCase()) return false;
      const priceVal = Number(String(p.price).replace(/,/g, '').match(/\d+/)) || 0;
      if (priceVal < selectedPrice.min || priceVal > selectedPrice.max) return false;
      if (selectedBeds !== 'Any' && Number(p.beds) < parseInt(selectedBeds)) return false;
      if (searchQuery && !String(p.location || '').toLowerCase().includes(searchQuery.toLowerCase())) return false;
      return true;
    });
  }, [houses, selectedType, selectedPrice, selectedBeds, searchQuery]);

  const paginated = filtered;

  const goBuy = () => navigate('/buy');
  const goRent = () => navigate('/rent');
  const setBoth = () => setListingType('');
    ...
    <div className="min-h-screen bg-white">
      <div className="max-w-[1400px] mx-auto px-4 md:px-8 pt-6 pb-4">
        ...
        <div className="text-sm text-gray-500 mb-2">
          ...
          <span className="font-semibold text-gray-700">Homes For Rent</span>
        </div>

        <h1 className="text-5xl font-serif text-gray-900 mb-6">Luxury Homes For Rent</h1>
        ...
        <div className="flex mb-6 max-w-3xl">
          ...
          />
          <button className="bg-white p-3 border border-gray-300 border-l-0 rounded-r text-gray-600 hover:bg-gray-50 transition">
            <Search size={20} />
          </button>
        </div>
        ...
        {/* Filter Buttons */}
        <div className="flex flex-wrap items-center gap-2 mb-4">
          <button onClick={goBuy} className={FILTER_BUTTON_CLASSES}>Buy</button>
          <button onClick={goRent} className="px-4 py-2 border border-gray-300 rounded text-sm font-medium text-gray-900 bg-gray-200">Rent</button>
          <button onClick={setBoth} className={listingType === '' ? 'px-4 py-2 border border-gray-300 rounded text-sm font-medium text-gray-900 bg-gray-200' : FILTER_BUTTON_CLASSES}>Both</button>
        ...
        {/* LIST + MAP */}
        <div className="flex flex-col md:flex-row max-w-[1400px] mx-auto pb-10">
          
          {/* LEFT SIDE – LISTINGS */}
          <div className="md:w-1/2 lg:w-2/5 pr-0 md:pr-6 space-y-6">
            {loading && <p className="text-gray-500 py-12 text-center">Loading...</p>}
            {!loading && paginated.map(p => <PropertyCard key={p.id} property={p} />)}
            {!loading && paginated.length === 0 && <p className="text-gray-500 py-12 text-center">No properties match your filters.</p>}
              ...
              <div className="flex justify-center gap-2 mt-6">
                {[...Array(meta.last_page)].map((_, i) => (
                  <button
                    ...
                    className={`px-3 py-1 border rounded ${pageNumber === i + 1 ? 'bg-gray-200' : ''}`}
                  >
                    {i + 1}
                  </button>
                ))}
          ...
          {/* RIGHT SIDE – MAP */}
          <div className="md:w-1/2 lg:w-3/5 h-[600px] border rounded-lg overflow-hidden mt-6 md:mt-0">
            <iframe 
